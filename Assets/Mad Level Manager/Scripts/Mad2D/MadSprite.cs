...
#region Actions
    ...
    /// <summary>
    /// Gets the world corners in this order: top-left, top-right, bottom-right, bottom-left
    /// </summary>
    /// <param name="arr"></param>
    public void GetWorldCorners(ref Vector3[] arr) {
        var bounds = GetBounds();
        var topLeft = transform.TransformPoint(new Vector3(bounds.xMin, bounds.yMax, transform.position.z));
        var topRight = transform.TransformPoint(new Vector3(bounds.xMax, bounds.yMax, transform.position.z));
        var bottomRight = transform.TransformPoint(new Vector3(bounds.xMax, bounds.yMin, transform.position.z));
        var bottomLeft = transform.TransformPoint(new Vector3(bounds.xMin, bounds.yMin, transform.position.z));
    ...
    public virtual Material GetMaterial() {
        ...
        if (!string.IsNullOrEmpty(shaderName) && setupShaderFunction != null) {
            ...
            if (hasPremultipliedAlpha) {
                if (panel.renderMode == MadPanel.RenderMode.DepthBased) {
                    material = panel.materialStore.MaterialFor(currentTexture, MadLevelManager.MadShaders.UnlitTintPreDepthBased);
                } else {
                    material = panel.materialStore.MaterialFor(currentTexture, MadLevelManager.MadShaders.UnlitTintPre);
                }
            } else {
                if (panel.renderMode == MadPanel.RenderMode.DepthBased) {
                    material = panel.materialStore.MaterialFor(currentTexture,
                        MadLevelManager.MadShaders.UnlitTintDepthBased);
                } else {
