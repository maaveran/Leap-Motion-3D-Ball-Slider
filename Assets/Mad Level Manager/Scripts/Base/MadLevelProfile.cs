...
public class MadLevelProfile {
        ...
        set {
            if (_backend != null) {
                if (_backend is DefaultBackend) {
                    Debug.LogError(
                        "The backend is already set to default. Most probably you're trying to override it too late!");
                } else {
                    Debug.LogWarning("The backend is already set. Are you trying to set it more than once?");
                }
            } else {
                _backend = value;
            }
    ...
    /// <summary>
    /// Initializes profile subsystem.
    /// For the most cases you don't need to do it. Explicit initialization may be needed if you want
    /// to access backend instance which is created only after the initialization or the first usage of
    /// MadLevelProfile.
    /// </summary>
...
//        }
        ...
        var levelsStr = backend.LoadProfile(profile);
    ...
    static void WriteProfile() {
        ...
        backend.SaveProfile(profile, levelsString);
    }
