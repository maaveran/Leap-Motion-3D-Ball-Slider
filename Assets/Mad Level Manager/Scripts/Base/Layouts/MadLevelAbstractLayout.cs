...
public abstract class MadLevelAbstractLayout : MadNode {
    ...
    public int enumerationOffset = 0;
    ...
    // works when useCurrentlyActiveConfiguration enabled
    public bool CurrentConfigurationValid() {
        if (!useCurrentlyActiveConfiguration) {
            Debug.LogWarning("Do not use this method when useCurrentyActiveConfiguration is set to false");
            return true;
        }
        ...
        if (string.IsNullOrEmpty(MadLevel.currentLevelName)) {
            // current level cannot be found in the configuration
            return false;
        }

        if (currentConfiguration.FindGroupById(configurationGroup) == null) {
            // cannot find currently selected group
            return false;
        }
    ...
    /// <summary>
    /// For the free layout works exactly the same as GetActiveIcon(). For the grid layout if
    /// there is only one icon per page, it will return it. With more icons, it will work as
    /// GetActiveIcon() method.
    /// </summary>
    ...
    public MadLevelIcon GetPreviousIcon(MadLevelIcon icon) {
        var nextLevel = currentConfiguration.FindPreviousLevel(icon.level.name);
        if (nextLevel == null) {
            return null;
        }
