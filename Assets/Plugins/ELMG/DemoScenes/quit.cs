...
public class quit:MonoBehaviour {
	...
	void OnGUI ()
	{
		...
		if (showGUI) {
			GUI.Box (new Rect (200, 140, 950, 400), "Anda Yakin ingin keluar dari permainan ?", customGui_txt);
			if (GUI.Button (new Rect (Screen.width/3+50,450 , 200, 60), "Yes ", customGui3))
				Debug.Log ("Clicked the button with text");
			if (GUI.Button (new Rect (Screen.width/3+250, 450, 200, 60), "No", customGui3))
				Debug.Log ("Clicked the button with text");
			...
			if(pxsLeapInput.GetHandAxis ("Rotation") < 0 ) {
				...
				if (GUI.Button (new Rect (Screen.width/3+50,450 , 200, 60), "Yes ", customGui4))
				if(a == 0){
					Application.Quit();
				}
			}
			if(pxsLeapInput.GetHandAxis ("Rotation") > 0) {
				...
				if (GUI.Button (new Rect (Screen.width/3+250, 450, 200, 60), "No", customGui4))
				if(a == 0){
					//Destroy(GameObject);
				}
			}
